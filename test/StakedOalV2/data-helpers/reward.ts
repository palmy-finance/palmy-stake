...
export const compareRewardsAtAction = async (
  ...
  const underlyingAsset = StakedOasysLendV2.address;
  // To prevent coverage to fail, add 5 seconds per comparisson.
  await increaseTime(5);

  const rewardsBalanceBefore = BigNumber.from(
    await (await StakedOasysLendV2.getTotalRewardsBalance(userAddress)).toString()
  );
  ...
  const assetConfiguration = assetConfig
    ...
    : {
        ...
        totalStaked: await StakedOasysLendV2.totalSupply(),
        underlyingAsset,
      };
  await StakedOasysLendV2.configureAssets([assetConfiguration]);

  const userBalance = await StakedOasysLendV2.balanceOf(userAddress);
  // Get index before actions
  const userIndexBefore = await getUserIndex(StakedOasysLendV2, userAddress, underlyingAsset);
  ...
  const userIndexAfter = await getUserIndex(StakedOasysLendV2, userAddress, underlyingAsset);
...
export const compareRewardsAtTransfer = async (
  ...
  const underlyingAsset = StakedOasysLendV2.address;
  const fromSavedBalance = await StakedOasysLendV2.balanceOf(fromAddress);
  const toSavedBalance = await StakedOasysLendV2.balanceOf(toAddress);
  ...
  // Get index before actions
  const fromIndexBefore = await getUserIndex(StakedOasysLendV2, fromAddress, underlyingAsset);
  const toIndexBefore = await getUserIndex(StakedOasysLendV2, toAddress, underlyingAsset);

  // Load actions that can or not update the user index
  const actions = () => [StakedOasysLendV2.connect(from.signer).transfer(toAddress, amount)];
  ...
  // Get index after actions
  const fromIndexAfter = await getUserIndex(StakedOasysLendV2, fromAddress, underlyingAsset);
  const toIndexAfter = await getUserIndex(StakedOasysLendV2, toAddress, underlyingAsset);
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (await StakedOasysLendV2.balanceOf(fromAddress)).toString();
    const toNewBalance = await (await StakedOasysLendV2.balanceOf(toAddress)).toString();
