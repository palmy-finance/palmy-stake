...
makeSuite('StakedToken V2. Redeem', (testEnv: TestEnv) => {
  ...
  it('User 1 stakes 50 Token', async () => {
    ...
    await waitForTx(await oalToken.connect(staker.signer).approve(stakedTokenV2.address, amount));
  ...
  it('User 1 activates the cooldown again, and tries to redeem a bigger amount that he has staked, receiving the balance', async () => {
    ...
    const tokenBalanceBefore = new BigNumber((await oalToken.balanceOf(staker.address)).toString());
    ...
    const tokenBalanceAfter = new BigNumber((await oalToken.balanceOf(staker.address)).toString());
